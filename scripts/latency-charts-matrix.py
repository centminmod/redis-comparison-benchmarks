...
def plot_latency_chart_single(all_data, metric_key, title_suffix, ylabel, out_filename, redis_io_threads, keydb_server_threads, dragonfly_proactor_threads, valkey_io_threads, requests, clients, pipeline, data_size):
    ...
    for i, db in enumerate(DBS):
        for j, height in enumerate(arr[i]):
            if height > 0:
                ax.annotate(f"{height:.2f}", xy=(x[j] + offsets[i], height), xytext=(0, 8), textcoords="offset points", ha="center", va="bottom", fontsize=8)  # Increased offset from 4 to 8
    ...
    for idx, thread_count in enumerate(thread_counts):
        ...
        for i, db in enumerate(DBS):
            ...
            for j, height in enumerate(values):
                if height > 0:
                    ax.annotate(f"{height:.2f}", xy=(x[j] + offsets[i], height), xytext=(0, 6), textcoords="offset points", ha="center", va="bottom", fontsize=9)  # Increased offset from 3 to 6
...
if __name__ == "__main__":
    ...
    print(f"[DEBUG] Running latency-charts-matrix.py with layout='{args.layout}'")
    ...
    for metric_key, ylabel in [("avg", "Average Latency (ms)"), ("p50", "p50 Latency (ms)"), ("p99", "p99 Latency (ms)")]:
        out_filename = f"{args.output_dir}/latency-{args.prefix}-{metric_key}-{args.layout}.png"
